...
def image_demo(input_path, output_path):
    ...
    for subdir in os.listdir(os.path.join(input_path)):
        ...
        tracks_active = []
        id = 0
        for yolotxt in sorted(os.listdir(os.path.join(input_path, subdir, 'gts'))):
            bboxes = []
            with open(os.path.join(input_path, subdir, 'gts', yolotxt), 'r') as f:
                for line in f.readlines():
                    ...
                    bboxes.append([xmin, ymin, xmax, ymax, 1.0, idClass])
            ...
            tracks_finished, tracks_active, id = track_iou(detections, tracks_active, 0.05, 0.4, 0.01, 0, 0, id)

...
def image_demo(input_path, output_path):
    ...
    model_l = attempt_load('yolov5m.pt', map_location=device)
    ...
    for subdir in os.listdir(os.path.join(input_path)):
        ...
        with open(os.path.join(output_path, subdir + '.txt'), 'w') as f:
            ...
            for line in lines:
                f.write(line + '\n')
    print("time:{},inference_time:{},nms_time:{},wbf_time:{},track_time:{}".format(time.time()-start_time, sum(inference_times),
                                                                                   sum(nms_times), sum(wbf_times), sum(track_times)))

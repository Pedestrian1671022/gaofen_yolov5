...
def image_demo(input_path, output_path):
    ...
    model_l = attempt_load('baseline_yolov5l.pt', map_location=device)
    stride = int(model_l.stride.max())
    imgsz = check_img_size(1920, s=stride)
    model_l(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model_l.parameters())))

    model_m = attempt_load('yolov5m.pt', map_location=device)
    model_m(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model_m.parameters())))
    ...
    for subdir in os.listdir(os.path.join(input_path)):
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            paths.append(path)
            nf = nf - 1
            if nf != 0 and len(paths) != 20:
                continue
            inference_start_time = time.time()
            pred_l = model_l(imgs, augment=False, visualize=False)[0]
            pred_m = model_m(imgs, augment=False, visualize=False)[0]
            ...
            for i, det in enumerate(zip(pred_l, pred_m)):
                bboxes_l = []
                bboxes_m = []
                det_l = det[0]
                det_m = det[1]
                ...
                im0 = im0s.copy()
                if len(det_l):
                    det_l[:, :4] = scale_coords(img.shape[2:], det_l[:, :4], im0.shape).round()
                    for xmin, ymin, xmax, ymax, conf, cls in reversed(det_l):
                ...
                if len(det_m):
                    det_m[:, :4] = scale_coords(img.shape[2:], det_m[:, :4], im0.shape).round()
                    for xmin, ymin, xmax, ymax, conf, cls in reversed(det_m):
                ...
                wbf_times.append(time.time() - wbf_start_time)
                ...
                for track_finished in tracks_finished:
                    ...
                    line = '{},{},{},{},{},{},{},{},{}'.format(int(path[-10:-4]), track_finished['id'], bbox[0] + 2,
                    ...
                    for _line in _lines[max(0, len(_lines) - 1000):]:
                        if _line.startswith('{},{},'.format(int(path[-10:-4]) - 1, track_finished['id'])):
                            flag_1 = True
                        if _line.startswith('{},{},'.format(int(path[-10:-4]) - 2, track_finished['id'])):
                            flag_2 = True
                        if _line.startswith('{},{},'.format(int(path[-10:-4]) - 3, track_finished['id'])):
                            flag_3 = True
                        if _line.startswith('{},{},'.format(int(path[-10:-4]) - 4, track_finished['id'])):
                            flag_4 = True
                    if not flag_1:
                        lines.append('{},{},{},{},{},{},{},{},{}'.format(int(path[-10:-4]) - 1, track_finished['id'],
                                                                         bbox[0] + 2, bbox[1] + 2, bbox[2] - bbox[0] - 4,
                                                                         bbox[3] - bbox[1] - 4, 1, 1,
                                                                         1))
                    if not flag_2:
                        lines.append('{},{},{},{},{},{},{},{},{}'.format(int(path[-10:-4]) - 2, track_finished['id'],
                                                                         bbox[0] + 2, bbox[1] + 2, bbox[2] - bbox[0] - 4,
                                                                         bbox[3] - bbox[1] - 4, 1, 1,
                                                                         1))
                    if not flag_3:
                        lines.append('{},{},{},{},{},{},{},{},{}'.format(int(path[-10:-4]) - 3, track_finished['id'],
                                                                         bbox[0] + 2, bbox[1] + 2, bbox[2] - bbox[0] - 4,
                                                                         bbox[3] - bbox[1] - 4, 1, 1,
                                                                         1))
                    if not flag_4:
                        lines.append('{},{},{},{},{},{},{},{},{}'.format(int(path[-10:-4]) - 4, track_finished['id'],
                                                                         bbox[0] + 2, bbox[1] + 2, bbox[2] - bbox[0] - 4,
                                                                         bbox[3] - bbox[1] - 4, 1, 1,
                                                                         1))
                    lines.append(line)
        ...
        with open(os.path.join(output_path, subdir + '.txt'), 'w') as f:
            ...
            for line in lines:
                f.write(line + '\n')
    print("time:{},inference_time:{},nms_time:{},wbf_time:{},track_time:{}".format(time.time() - start_time,
                                                                                   sum(inference_times),
                                                                                   sum(nms_times), sum(wbf_times),
                                                                                   sum(track_times)))
